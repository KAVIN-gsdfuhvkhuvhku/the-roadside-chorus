...
#black colour rect for the two transitions
...
func _ready() -> void:
	...
	transition_layer.layer = 100 #-> its on the TOP
	transition_rect = ColorRect.new()
	transition_rect.color = Color.BLACK
	#make the colour rect fill its parent (the Canvaslayer, which fills the viewport)
	transition_rect.anchor_right = 1
	transition_rect.anchor_bottom = 1
	#hiding it for now and making it come back when we need to transition
...
func _slide_in():
	...
	#set the initial position to the right of the screen
	var viewport_size = get_viewport_rect().size
